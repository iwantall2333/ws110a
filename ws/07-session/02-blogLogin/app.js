...
router.get('/', list)
  .get('/login', loginUi)
  .post('/login', login)  //方法設置不同 : get取'/login' 與 post取'/login'
  .get('/logout', logout)
  .get('/post/new', add)
  .get('/post/:id', show)
  .post('/post', create)
...
async function login(ctx) {
  ...
  if (body.type === "form") {
    ...
    if (dbUser.password === user.password) {
      ctx.state.session.set('user', user)                                 //登入成功 則用session記住user  因session是跟著瀏覽器，所以同時AB連入是沒關係 是兩個不同session
...
async function logout(ctx) {                //登出 : 忘記user變數
