...
router.get('/show', session.initMiddleware(), list)
  .get('/signup', signupUi)
  ...
  .get('/post', session.initMiddleware(),show)
...
async function signup(ctx) {
  ...
  if (body.type === "form") {
    ...
    if (dbUsers.length === 0) {
      sqlcmd("INSERT INTO users (username, password, email) VALUES (?, ?, ?)", [user.username, hex_md5(user.password), user.email]);
...
async function login(ctx) {
  ...
  if (body.type === "form") {
    ...
    if (dbUser.password === hex_md5(user.password) && user.username!="") {
      ctx.state.session.set('user', user)
      //console.log('session.user=', await ctx.state.session.get('user'))
      ctx.response.redirect('/show');
    } else {
      ctx.response.body = render.fail()
    }
  }
